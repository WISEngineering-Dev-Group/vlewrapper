 ...
 */
View.prototype.viewStudentAssets = function(args) {
	var launchNode;
	if (args != null) {
		launchNode = args[0];
	}
	...
	if($('#studentAssetsDiv').size()==0){
		...
		var assetEditorDialogHtml;
		if (launchNode != null && launchNode.type == "DesignWallNode"){
			assetEditorDialogHtml = "<div id='studentAssetEditorDialog' style='display: none; text-align:left;'><div style='margin-bottom:.5em;'>" 
			...
			+ "<input style='margin:0 .5em;' type='file' size='30' id='uploadAssetFile' name='uploadAssetFile' onchange=\"eventManager.fire('designWallUploadImage')\"></input>"
	...
	var insertImage = function() {
		// code here to insert html containing path of selected file...
		// window.alert('Ready to inject selected file');
		var parent = document.getElementById('assetSelect');
		var ndx = parent.selectedIndex;
		if(ndx!=-1){
			var opt = parent.options[parent.selectedIndex];
			...
			var getStudentUploadsBaseUrl = view.config.getConfigParam("getStudentUploadsBaseUrl");
			var fileWWW = getStudentUploadsBaseUrl + "/" + workgroupId + "/" + name;
			//window.alert('file url: ' + fileWWW);
			launchNode.insertImageUrl(fileWWW);
			$('#studentAssetsDiv').dialog('close');			
		} else {
			window.alert('cannot find selected file');
		}
		
	};

	if ((launchNode != null) && (launchNode.type == "DesignWallNode")) {
		$('#studentAssetsDiv').dialog({
			autoOpen:false,
			closeText:'',
			resizable:false,
			width:600,
			position:['center',50],
			modal:false,
			title:this.i18n.getString("student_assets_my_files",this.config.getConfigParam("locale")), 
			buttons:[
				{text:'Insert Selected File',click:insertImage},
				{text:deleteSelectedFileText,click:remove},
				{text:doneText,click:done}
				]
			});
	}
	else {	// default WISE4 behavior
	...
	}	// launchNode test for DesignWallNode
...
View.prototype.designWallImageUploaded = function(e){
	//window.alert("designWallImageUploaded");
	...
	if(frame.document && frame.document.body && frame.document.body.innerHTML != ''){
		...
		var filename = document.getElementById('uploadAssetFile').value;
		

		// build url for uploaded file
		var view = window.WISEng_vle;
		var workgroupId = view.userAndClassInfo.getWorkgroupId();
		var getStudentUploadsBaseUrl = view.config.getConfigParam("getStudentUploadsBaseUrl");
		...
		currentNode.insertImageUrl(fileWWW);

		// close the assets panel
		$('#studentAssetsDiv').dialog('close');	
 ...
 */
View.prototype.studentAssetSubmitUpload = function(args) {
	if (args != null) {
		launchNode = args[0];
	}
	...
	if(filename && filename != ''){
		...
		if(!view.utils.fileFilter(view.allowedStudentAssetExtensions,filename)){
			...
			var designWall = true;
			if (designWall) {
				frame.addEventListener('load',view.designWallImageUploaded,false);
			}
			else {
