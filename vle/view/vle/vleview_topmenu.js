View.prototype.dropDownMenuDispatcher = function(type,args,obj){
	if(type=='showAllWork'){
		...
		obj.viewStudentAssets(args);
	} else if(type=='studentAssetSubmitUpload') {
		obj.studentAssetSubmitUpload(args);
...
View.prototype.displayShowAllWork = function() {
	...
	if(this.annotationsRetrieved && this.projectMetaDataRetrieved) {
	    ...
	    if($('#showallwork').size()==0){
	    	...
	    	$('<div id="showallwork"></div>').dialog({autoOpen:false,closeText:'',width:'96%',height:(document.height * .96),modal:true,title:'Design Journal (with Teacher Feedback and Scores)'});
	    }	    
