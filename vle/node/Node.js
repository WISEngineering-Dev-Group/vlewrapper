/* WISEngineering stuff */
/* Node.js starts after the WISEng functions */
// WISEngineering global variables
var WISEng_vle;	// the VLE -- assigned in the Node constructor
var WISEng_portfolioRequest;

function WISEng_createRequest() {
    try {
...
/* the function OpenWindowWithPost() was is a modified version of code at:
http://taswar.zeytinsoft.com/2010/07/08/javascript-http-post-data-to-new-window-or-pop-up/
*/
function WISEng_OpenWindowWithPost(url, windowoption, name, params) {
	var windowOpenString = "window.open('','"
		+ name + "','" + windowoption + "')";
	var form = document.createElement("form");
	form.setAttribute("method", "post");
	form.setAttribute("action", url);
	form.setAttribute("target", name);
	//form.setAttribute("onsubmit", windowOpenString);
...
function WISEng_DesignPortfolio(userName, jsessid) {
//    window.open("http://localhost:8080/wiseng/designPortfolio?uid=" + userName,
//        "Design Portfolio", "width=800,height=600,left=100,top=100,resizable=yes,scrollbars=yes,menubar=no");
	var params = { 'userName' : userName };
	WISEng_OpenWindowWithPost("/wiseng/designPortfolio", "width=800,height=600,left=100,top=100,resizable=yes,scrollbars=yes,menubar=no", "Design Portfolio", params);
//	eventManager.fire("showJournal");
}

function WISEng_addPortfolioEntry(nodeId) {
	var wiseng_runId = WISEng_vle.config.getConfigParam('runId');
	var wiseng_latestState = WISEng_vle.getLatestStateForNode(nodeId);
	var wiseng_node = WISEng_vle.project.getNodeById(nodeId);
	var wiseng_userName = WISEng_vle.userAndClassInfo.getUserLoginName();
//	window.alert("runId: " + wiseng_runId + "\n"
//		+ "nodeId: " + nodeId + "\n"
//		+ "userName: " + wiseng_userName);
	WISEng_portfolioRequest = WISEng_createRequest();
	if (WISEng_portfolioRequest == null) {
		...
		var requestData = "userName=" + escape(wiseng_userName)
			...
			+ "&mode=" + "add";
		WISEng_portfolioRequest.onreadystatechange = function() {
			if (WISEng_portfolioRequest.readyState == 4
				&& WISEng_portfolioRequest.status == 200) {
				var responseText = WISEng_portfolioRequest.responseText;
				//window.alert(responseText);

				// received json response, get display index
				var jsonResponse = JSON.parse(responseText);
				var pageToDisplay = jsonResponse.displayIndex;
				//window.alert("displayIndex:" + pageToDisplay);

				eventManager.fire("showJournal", pageToDisplay);
			}
		};
		WISEng_portfolioRequest.open("POST", url, true);
		WISEng_portfolioRequest.setRequestHeader("Content-Type",
			"application/x-www-form-urlencoded");
		WISEng_portfolioRequest.send(requestData);
	}
}
...
Node.prototype.getShowAllWorkHtml = function(vle, divIdPrefix){
    ...
    if (nodeVisitArray.length > 0) {
        ...
        if(latestState!=null){
			...
			// debug
			this.wiseng_nodeId = this.getNodeId();
			this.wiseng_latestState = vle.getLatestStateForNode(this.wiseng_nodeId);
			this.wiseng_node = vle.project.getNodeById(this.wiseng_nodeId);
			this.wiseng_userName = vle.userAndClassInfo.getUserLoginName();

        		showAllWorkHtmlSoFar += '<div class=\"showallLatest\">Latest Work:' 
			+ '<button onclick=\"WISEng_addPortfolioEntry(\''
			+ this.getNodeId()
			+ '\');\">Add to Portfolio</button>' 
			+ '</div>'

        		+ '<div id=\"'+divId+'\" contentBaseUrl=\"'+contentBaseUrl+'\" class=\"'+divClass+'\" style=\"'+divStyle+'\">' 
//			+ 'Node ID:' + this.getNodeId() + '<br />'
//			+ 'User Login:' + vle.userAndClassInfo.getUserLoginName() + '<br />'
//			+ 'WISEng Login:' + this.wiseng_userName + '<br />'
//			+ 'RunId:' + vle.config.getConfigParam('runId') + '<br />'
//			+ 'Node Data:' + this.wiseng_node.translateStudentWork(this.wiseng_latestState.getStudentWork()) + '<br />'
			+ this.translateStudentWork(latestState.getStudentWork()) + '</div>';
        	}
